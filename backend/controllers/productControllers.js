...
const { name, price, artist, category, countInStock, description } = req.body;

if(!name|| !file || !price|| !artist || !category || !countInStock || !description  ) {
...
let q =  "INSERT INTO products (name, image, filename, price, artist, category, countInStock, description) VALUES (?,?,?,?,?,?,?,?)"

 await db.query(q, [name, file, filename, price, artist, category, countInStock, description], (err,result) => {
   if(err) {
...
const q = `SELECT count(*) OVER() AS full_count, products.product_id, products.product_id, products.name, products.price, products.image, products.artist, products.category, products.countInStock, products.description, ROUND (IFNULL(avg(reviews.rating), 0),2) AS averageRating, COUNT(reviews.product_id) AS numOfReviews  
FROM products  
LEFT OUTER JOIN reviews 
ON products.product_id=reviews.product_id
...
const updateProduct = asyncWrapper (async(req,res,next) => {
 ...
 const { name, price, artist, category, countInStock, description } = req.body;
   ...
   if(req.file) {
    ...
    let q = 'UPDATE products SET name=?, image=?, price=?, artist=?, category=?, countInStock=?, description=? WHERE product_id = ?';

await db.query(q,  [name, file, price, artist, category, countInStock, description, id], (err,result) => {
if(err) {
  console.log(err)
} else {
  console.log(result)
  res.status(201).json({ result })
    ...
    let q = 'UPDATE products SET name=?, price=?, artist=?, category=?, countInStock=?, description=? WHERE product_id = ?';

    await db.query(q,  [name, price, artist, category, countInStock, description, id], (err,result) => {
    if(err) {
      console.log(err)
    } else {
      console.log(result)
...
const validateIsAdmin = asyncWrapper(async(req, res, next) => {

  let adminTrue = req.user.isAdmin
